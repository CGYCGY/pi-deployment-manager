...
— `EXPOSE` → port, `VOLUME` → one volume per declared path, `HEALTHCHECK` URL → the health probe — so a plain
...
`sync_storages` adds mounts to an app provisioned before a path was declared (effective next deploy,
mounts empty).
